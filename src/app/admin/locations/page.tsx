...
export default function AdminLocationsPage() {
  ...
  const validate = (): boolean => {
    ...
    if (form.name.trim().length > 100) e.name = "Location name must be 100 characters or less";
    ...
    if (form.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email)) e.email = "Must be a valid email";
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setDialogOpen(false);
      showToast("success", isEdit ? "Location updated successfully" : "Location created successfully");
      fetchLocations();
    } catch (err) {
  ...
  const handleDelete = async (location: Location) => {
    ...
    if (totalBookings > 0) {
      showToast("error", `Cannot delete "${location.name}" — ${totalBookings} booking(s) are linked to it.`);
      return;
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">{total} pickup & dropoff zones</p>
        </div>
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16">
            ...
            <p className="text-muted-foreground text-sm mt-1">
              {search ? "Try a different search term" : "Click \"Add Location\" to create your first pickup/dropoff zone"}
            </p>
          </CardContent>
        </Card>
      ) : (
          ...
          <div className="space-y-4 pt-2">
            ...
            <div className="flex items-center gap-6">
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                />
                <span className="text-sm font-medium">Airport location</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
