...
export default function SignInPage() {
  ...
  const handleSendCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : t("sendCodeFailed");
      setError(message);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleVerifyCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.status === "complete") {
        ...
        setError(t("signInIncomplete", { status: String(result.status) }));
      }
    } catch (err: unknown) {
  ...
  if (step === "otp") {
    return (
      <div className="space-y-6">
        <div className="text-center">
          <h1 className="text-2xl font-bold">{t("enterVerificationCode")}</h1>
          <p className="text-muted-foreground mt-1">
