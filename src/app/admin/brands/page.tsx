...
export default function AdminBrandsPage() {
  ...
  const validate = (): boolean => {
    ...
    if (form.name.trim().length > 100) e.name = "Brand name must be 100 characters or less";
    if (form.logo && !/^https?:\/\/.+/i.test(form.logo)) e.logo = "Must be a valid URL";
    setErrors(e);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setDialogOpen(false);
      showToast("success", isEdit ? "Brand updated successfully" : "Brand created successfully");
      fetchBrands();
    } catch (err) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">{total} brands in your system</p>
        </div>
