...
export default function DashboardSettingsPage() {
  ...
  if (!profile) {
    return <div className="text-center py-20 text-muted-foreground">{t("failedToLoadProfile")}</div>;
  }
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        ...
        <p className="text-muted-foreground mt-1">{t("accountSettingsSubtitle")}</p>
      </div>
        ...
        <CardContent className="p-6 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div>
              ...
              <p className="font-medium">{profile.isEmailVerified ? tc("yes") : tc("no")}</p>
            </div>
            <div>
      ...
      <div className="flex items-center gap-3">
        ...
        {saved && <span className="text-sm text-emerald-600">{tc("savedSuccessfully")}</span>}
      </div>
    </div>
  );
}
