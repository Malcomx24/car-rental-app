        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-sm text-muted-foreground">({car.totalReviews} reviews)</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-5 gap-4">
              {[
                { icon: <Fuel className="h-5 w-5" />, label: "Fuel", value: FUEL_LABELS[car.fuelType] || car.fuelType },
                { icon: <Settings2 className="h-5 w-5" />, label: "Transmission", value: car.transmission.replace("_", " ") },
                { icon: <Users className="h-5 w-5" />, label: "Seats", value: `${car.seats} seats` },
                { icon: <Palette className="h-5 w-5" />, label: "Color", value: car.color },
                { icon: <Gauge className="h-5 w-5" />, label: "Mileage", value: `${car.mileage.toLocaleString()} mi` },
              ].map((spec) => (
                <Card key={spec.label}>
                ...
                <CardContent className="p-6 space-y-5">
                    ...
                    <div className="space-y-2 text-sm">
                      {car.weekendPricePerDay && (
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">${Number(car.weekendPricePerDay).toFixed(0)}/day</span>
                        </div>
                      )}
                      {car.weeklyPrice && (
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Weekly rate</span>
                          <span className="font-medium">${Number(car.weeklyPrice).toFixed(0)}/week</span>
                        </div>
                      )}
                      {car.monthlyPrice && (
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Monthly rate</span>
                          <span className="font-medium">${Number(car.monthlyPrice).toFixed(0)}/month</span>
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  {car.securityDeposit > 0 && (
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      ...
                      <span>${Number(car.securityDeposit).toFixed(0)} refundable security deposit</span>
                    </div>
                  )}
                  ...
                  <div className="space-y-3 pt-3 border-t text-sm">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium">Full Insurance Available</p>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">Up to 24h before pickup</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      <Calendar className="h-5 w-5 text-primary shrink-0" />
                      <div>
                        <p className="font-medium">{car.totalBookings} bookings</p>
                        <p className="text-xs text-muted-foreground">This vehicle is popular</p>
                      </div>
                    </div>
                  </div>
                </CardContent>
