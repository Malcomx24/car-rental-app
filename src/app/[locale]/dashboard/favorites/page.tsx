...
export default function DashboardFavoritesPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">{t("favoriteCars")}</h1>
        <p className="text-muted-foreground mt-1">{favorites.length} {favorites.length !== 1 ? t("savedVehiclesPlural") : t("savedVehicles")}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-xl font-medium">{t("noFavoritesYet")}</p>
          <p className="text-muted-foreground mt-1">{t("browseAndSave")}</p>
          <Link href="/cars"><Button className="mt-4">{t("browseCars")}</Button></Link>
        </div>
      ) : (
