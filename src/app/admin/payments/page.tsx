...
import { Search, MoreHorizontal, ChevronLeft, ChevronRight, Loader2, DollarSign, CreditCard, TrendingUp, ArrowDownRight, RotateCcw } from "lucide-react";
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b bg-muted/50">
                  <th className="text-left p-4 text-sm font-medium">Transaction</th>
                  <th className="text-left p-4 text-sm font-medium hidden md:table-cell">Booking</th>
                  <th className="text-left p-4 text-sm font-medium">Amount</th>
                  <th className="text-left p-4 text-sm font-medium">Status</th>
                  <th className="text-left p-4 text-sm font-medium hidden lg:table-cell">Date</th>
                  ...
                  payments.map((p) => (
                    <tr key={p.id} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                      <td className="p-4">
                        ...
                        <p className="text-xs text-muted-foreground">{p.description || "N/A"}</p>
                      </td>
                      ...
                      <td className="p-4">
                        ...
                        {p.refundAmount && p.refundAmount > 0 && (
                          <p className="text-xs text-red-600">Refunded: {formatCurrency(p.refundAmount)}</p>
                        )}
                      </td>
                      <td className="p-4">
