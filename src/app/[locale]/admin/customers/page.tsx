...
import {
  Search, ChevronLeft, ChevronRight, Loader2, Users, UserCheck, TrendingUp, DollarSign,
  Phone, Calendar, Shield, Star, Eye, X,
} from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">{t("viewAndManageCustomers")}</p>
      </div>
          ...
          <Card>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">{t("totalCustomersLabel")}</p>
              </div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">{t("activeCustomersLabel")}</p>
              </div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">{t("totalBookingsStatLabel")}</p>
              </div>
            </CardContent>
          </Card>
          <Card>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b bg-muted/50">
                  <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase">{t("customer")}</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase">{t("role")}</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase">{t("bookingsColumn")}</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase">{t("reviewsColumn")}</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase">{t("statusLabel")}</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase">{t("joinedColumn")}</th>
                ...
                {loading ? (
                  ...
                  customers.map((c) => (
                    <tr key={c.id} className="hover:bg-muted/30 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${roleConfig[c.role]?.className || "bg-gray-100 text-gray-800"}`}>
      ...
      {(selectedCustomer || detailLoading) && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setSelectedCustomer(null)}>
          <div className="bg-background rounded-xl shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            {detailLoading ? (
                  ...
                  <div className="flex items-center gap-3">
                    <div className="h-12 w-12 rounded-full bg-primary/10 flex items-center justify-center text-lg font-bold text-primary">
                      {selectedCustomer.avatar ? (
                        <img src={selectedCustomer.avatar} alt="" className="h-12 w-12 rounded-full object-cover" />
                      ) : (
                        `${selectedCustomer.firstName[0]}${selectedCustomer.lastName[0]}`.toUpperCase()
                      )}
                    </div>
                    <div>
                      <h2 className="text-lg font-semibold">{selectedCustomer.firstName} {selectedCustomer.lastName}</h2>
                      <p className="text-sm text-muted-foreground">{selectedCustomer.email}</p>
                    </div>
                  </div>
                  <button onClick={() => setSelectedCustomer(null)} className="p-2 rounded-md hover:bg-muted">
                    <X className="h-5 w-5" />
                  </button>
                </div>
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${roleConfig[selectedCustomer.role]?.className || ""}`}>
                        {roleConfig[selectedCustomer.role]?.label || selectedCustomer.role}
                      </span>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                      <Calendar className="h-4 w-4 text-muted-foreground" />
                      <span>{t("memberSince")} {new Date(selectedCustomer.createdAt).toLocaleDateString()}</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                      <DollarSign className="h-4 w-4 text-muted-foreground" />
                      <span className="font-medium">{formatCurrency(selectedCustomer.totalSpent)} {t("spent")}</span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="text-center p-3 bg-muted/50 rounded-lg">
                      <p className="text-xl font-bold">{selectedCustomer._count.bookings}</p>
                      <p className="text-xs text-muted-foreground">{t("bookings")}</p>
                    </div>
                    <div className="text-center p-3 bg-muted/50 rounded-lg">
                      <p className="text-xl font-bold">{selectedCustomer._count.reviews}</p>
                      <p className="text-xs text-muted-foreground">{t("reviews")}</p>
                    </div>
                    <div className="text-center p-3 bg-muted/50 rounded-lg">
                      <p className="text-xl font-bold">{selectedCustomer._count.favorites}</p>
                  ...
                  {selectedCustomer.bookings.length > 0 && (
                    <div>
                      <h3 className="font-semibold text-sm mb-3">{t("recentBookings")}</h3>
                      <div className="space-y-2">
                        {selectedCustomer.bookings.map((b) => (
                          <div key={b.id} className="flex items-center justify-between p-3 bg-muted/30 rounded-lg text-sm">
                            <div>
                            ...
                            <div className="text-right">
                              ...
                              <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium ${bookingStatusConfig[b.status]?.className || ""}`}>
                  ...
                  {selectedCustomer.reviews.length > 0 && (
                    <div>
                      <h3 className="font-semibold text-sm mb-3">{t("recentReviews")}</h3>
                      <div className="space-y-2">
                        {selectedCustomer.reviews.map((r) => (
                          <div key={r.id} className="p-3 bg-muted/30 rounded-lg text-sm">
                            <div className="flex items-center gap-2">
                              <div className="flex">
                              ...
                              <span className="font-medium text-xs">{r.car.name}</span>
                            </div>
                            {r.title && <p className="mt-1 text-muted-foreground">{r.title}</p>}
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <div className="flex gap-3 pt-2 border-t">
                    ...
                    >
                      {selectedCustomer.isActive ? t("deactivateAccount") : t("activateAccount")}
                    </Button>
                  </div>
                </div>
              </>
