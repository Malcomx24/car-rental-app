...
export default function AdminCategoriesPage() {
  ...
  const fetchCategories = useCallback(async () => {
    ...
    try {
      ...
      showToast("error", t("failedToFetchCategories"));
    } finally {
      setLoading(false);
    }
  }, [search, page, t]);
  ...
  const validate = (): boolean => {
    ...
    if (!form.name.trim()) e.name = t("validationCategoryNameRequired");
    if (form.name.trim().length > 100) e.name = t("validationCategoryNameMax");
    if (form.description && form.description.length > 500) e.description = t("validationDescriptionMax");
    setErrors(e);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok) {
        throw new Error(json.error || t("failedToSaveCategory"));
      }

      setDialogOpen(false);
      showToast("success", isEdit ? t("categoryUpdated") : t("categoryCreated"));
      fetchCategories();
    } catch (err) {
      const msg = err instanceof Error ? err.message : t("failedToSaveCategory");
      showToast("error", msg);
    } finally {
      setSaving(false);
    }
  ...
  const handleDelete = async (category: Category) => {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || t("failedToDeleteCategory"));
      showToast("success", t("categoryDeleted"));
      fetchCategories();
    } catch (err) {
      const msg = err instanceof Error ? err.message : t("failedToDeleteCategory");
      showToast("error", msg);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">{total} {t("categoriesInSystem")}</p>
        </div>
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16">
            ...
            <p className="text-muted-foreground text-sm mt-1">
              {search ? t("noResultsSearchDifferent") : t("clickToAddFirst") + " \"" + t("addCategory") + "\" " + t("toCreateFirst") + " " + t("category")}
            </p>
          </CardContent>
        </Card>
      ) : (
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-xs text-muted-foreground">
                      ...
                      <Badge variant="secondary" className="text-xs">
                        {category._count.cars} {category._count.cars === 1 ? t("car") : t("carsCount")}
                      </Badge>
                    </div>
                  </div>
                </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={(o) => !o && setDialogOpen(false)}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 pt-2">
            ...
            <div>
              <Label>{t("iconTag")}</Label>
              <Input
                value={form.icon}
                onChange={(e) => setForm((p) => ({ ...p, icon: e.target.value }))}
                placeholder={t("placeholderIconTag")}
                className="mt-1"
              />
